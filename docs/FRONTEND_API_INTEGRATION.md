...
const API_BASE_URL = process.env.NODE_ENV === 'production' 
  ? 'https://api.yourdomain.com/api'  // Замените на ваш домен после деплоя
...
> **💡 Важно:** После деплоя бэкенда на сервер с доменом, обновите `API_BASE_URL` на production URL.  
> См. `DEPLOYMENT_GUIDE.md` для подробных инструкций по деплою бэкенда.
...
**Важно:** После деплоя бэкенда на сервер с доменом, фронтенд-разработчик должен использовать production URL. См. `DEPLOYMENT_GUIDE.md` для инструкций по деплою.
...
- [ ] Настроить базовый URL API через переменную окружения `REACT_APP_API_URL`
